...
export class TaskDispatcher {
  ...
  public createTask(executionId: string, taskType: string): TaskRecord {
  ...
  public acquireLease(taskId: string, workerId: string, leaseDurationMs: number = 30000): TaskRecord {
  ...
  public updateHeartbeat(taskId: string, workerId: string, leaseExtensionMs: number = 30000): void {
    const task = this.taskMap.get(taskId);
  ...
  public completeTask(taskId: string, workerId: string): TaskRecord {
    const task = this.taskMap.get(taskId);
    if (!task || task.lease_owner !== workerId) {
      throw new AgexError({
        code: 'INVALID_LEASE_OWNER',
